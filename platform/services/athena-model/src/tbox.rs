...
// athena-deploy-model's retirement section executes the staged delete. Same
...
/// athena-make's name→route derivation (lower + plural, with its irregulars).
...
/// (never a file — DECLARED⊃CLAIMED⊃SERVED); `served_routes` = athena-make's live
...
mod retire_tests {
    ...
    #[test]
    fn route_derivation_matches_athena_make_irregulars() {
