...
/// Files that are REGENERATED by the toolchain — a rebase conflict on one carries
/// no human decision: origin/main's side is always correct (the next generation
/// pass recreates any werk-side delta). #3632: this exact conflict (doc-coherence.md)
/// wedged both #3431's and #2588's resumed lands. #3638: the list itself now lives
/// in werk-teardown (shared with the teardown generated-churn carve-out) — ONE list,
/// two consumers, no drift.
const GENERATED_FILES: &[&str] = werk_teardown::GENERATED_FILES;
...
fn commit_inner(card: u64, role: &str, summary: &str, home: &Path, werk_base: &Path, rebase: bool) -> R<String> {
    ...
    let ahead = run_in(&werk_s, "git", &["rev-list", "--count", "origin/main..HEAD"])
        ...
        .parse::<u64>()
        .unwrap_or(0);
    // Nothing to commit AND nothing already committed -> two distinct states (#3632):
    //
    //   RESUME: the card's work already LANDED on origin/main (an acp/squash commit
    //   referencing #<card> — the same Tier-1 merge proof werk-teardown uses, #3014).
    //   A clean werk here is the SUCCESS state of a completed commit step; refusing
    //   killed three resumed lands in one week (#3624/#3431/#2588) before the
    //   pipeline's idempotent merge/accept verbs were ever reached.
    //
    //   GENUINE NO-OP: no card work anywhere — refuse loudly, exactly as before.
    if !dirty && ahead == 0 {
        let subjects = run_in(&werk_s, "git", &["log", "origin/main", "-n", "300", "--format=%s"])
            .unwrap_or_default();
        if werk_teardown::subjects_reference_card(&subjects, card) {
            let sha = run_in(&werk_s, "git", &["rev-parse", "HEAD"])?.trim().to_string();
            jsonl(home, role, card, &trace, "commit.idempotent",
                &format!(",\"reason\":\"work-already-on-main\",\"sha\":\"{}\"", sha));
            emit_spine(home, "commit.idempotent", role, card, &trace,
                &[("disposition", "pass"), ("reason", "work-already-on-main"), ("sha", &sha)]);
            return Ok(sha);
        }
