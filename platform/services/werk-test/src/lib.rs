...
///
/// The card's complaint: 155 registered tests neither ran nor said why, and a
/// bare count nobody can act on sits in the report every morning. Each of the
/// four states below is decided from something we can look at — the file on
/// disk, the executed ledger, the lane table — never from a declaration.
///
/// `LaneSilent` is a fifth state the card's AC did not name, and it is here
/// because the evidence produces it: a file that exists, whose lane exists,
/// for which the lane emitted nothing. Folding that into `NoLane` would be a
/// lie about the lane table, and folding it into `Dead` a lie about the tree.
/// It is the "the unit died empty" case the nightly already attributes.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord)]
pub enum GapState {
    /// The file is not in the tree. The registration outlived its test.
    Dead,
    /// The file exists and its lane emitted cases for it — but never under this
    /// name. The registry and the runner disagree about what the test is called.
    NameMismatch,
    /// No runner covers this path: `unit_of_path` has no unit for it. Nothing
    /// could have run it, so "never ran" is accurate and the lane table is the bug.
    NoLane,
    /// The file exists, a lane covers it, and the lane emitted nothing for it.
    /// The unit produced no results — fix the unit, not the ledger.
    LaneSilent,
}
...
impl GapState {
    pub fn label(self) -> &'static str {
        match self {
            GapState::Dead => "DEAD",
            GapState::NameMismatch => "NAME MISMATCH",
            GapState::NoLane => "NO LANE",
            GapState::LaneSilent => "LANE SILENT",
        }
    }
}
...
///
/// #4106 — `unit_of_path` alone was the wrong predicate and said so out loud:
/// it called the 13 `proving/flows/*.spec.cjs` specs NO LANE while the browser
/// lane runs them every time it fires. That lane does not select by path — it
/// selects by `testConcern == "ui"` in the registry (`ui_files`), so the set of
/// ui-registered files is the second half of the evidence. A predicate that
/// cannot see one of the two selectors cannot answer the question.
pub fn has_lane(path: &str, ui_registered: &std::collections::BTreeSet<String>) -> bool {
...
pub fn join_cases(
    ...
    test_names: &[String],
    entities: &[String],
) -> (Vec<(CaseResult, String, String)>, usize) {
    ...
    for c in cases {
        if let Some(e) = index.get(&(c.file_path.as_str(), c.test_name.as_str())) {
            // #4271 — the join key is the case's own name here, reported
            // explicitly. An empty third element would make the census join on
            // "" for every ordinary case and read all 8,749 as never-ran.
            joined.push((c.clone(), (*e).to_string(), c.test_name.clone()));
            continue;
        }
        ...
        match hits.first() {
            ...
            Some((reg_name, entity)) => {
                // #4271 — the case KEEPS its emitted name. It used to be
                // rewritten to the registered one right here, which is how 42
                // cases generated by `describe.each` from ONE literal `it()`
                // became 42 rows with one name and one ofTest. The registered
                // row still rides in `entity` (ofTest), and the registered
                // NAME rides beside it as the join key: identity and join are
                // two facts on one record instead of one overwriting the other.
                joined.push((c.clone(), (*entity).to_string(), (*reg_name).to_string()));
            }
        }
    }
    (joined, unjoined)
...
mod describe_prefix_join_4015 {
    ...
    fn negative_proof_a_describe_prefixed_case_joins_its_registered_identity() {
        ...
        // #4271 — the join key is the REGISTERED name, so run-to-run keys stay
        // stable; the case KEEPS the name it ran under, so a dynamic case is
        // distinguishable from its siblings.
        assert_eq!(joined[0].2, "getRecent default filters hidden");
        assert_eq!(joined[0].0.test_name, "MessageRouter — ingest basics getRecent default filters hidden");
    }
    ...
    fn the_longest_suffix_wins_because_it_is_the_it_name() {
        ...
        assert_eq!(joined[0].2, "also saves the file",
            "the join key is the REGISTERED identity");
        assert_eq!(joined[0].0.test_name, "suite also saves the file",
            "#4271 — and the case keeps the name it actually ran under");
    }
    ...
    fn duplicate_registered_names_collapse_so_no_tie_is_reachable() {
        ...
        assert_eq!(joined[0].2, "saves the file");
        assert_eq!(joined[0].0.test_name, "suite saves the file");
    }
...
mod never_ran_causes_4063 {
    ...
    fn k(f: &str, n: &str) -> (String, String) {
        (f.to_string(), n.to_string())
    }

    /// No file registered with the ui concern — the default for fixtures that
    /// are not about the browser lane.
    fn no_ui() -> std::collections::BTreeSet<String> {
        std::collections::BTreeSet::new()
    }

    /// The four states, one file each, against a tree we declare rather than
    /// one on disk — so the proof is about the RULES, not about this checkout.
    fn four_state_fixture() -> (Vec<(String, String)>, Vec<(String, String)>, Vec<String>) {
        let gap = vec![
            k("platform/api/tests/deleted.test.ts", "gone"),
            k("platform/api/tests/renamed.test.ts", "the registry's name"),
            k("docs/notes/orphan.test.js", "nothing runs .js under docs"),
            k("platform/api/tests/silent.test.ts", "unit died empty"),
        ];
        // the ledger spoke about renamed.test.ts, under a different name
        let executed = vec![k("platform/api/tests/renamed.test.ts", "the runner's name")];
        // everything but deleted.test.ts is present in the tree
        let on_disk = vec![
            "platform/api/tests/renamed.test.ts".to_string(),
            "docs/notes/orphan.test.js".to_string(),
            "platform/api/tests/silent.test.ts".to_string(),
        ];
...
mod athena_join_4136 {
    ...
    /// #4271 — a dynamic case keeps its OWN identity. `describe.each` /
    /// `test.each` generate N cases from ONE literal `it()` in source, so the
    /// registry can only ever hold the bare it-name. The join used to rewrite
    /// every emitted fullName to that bare name, so on the 2026-09-22 03:00
    /// run cards-api-permutation.test.ts produced 42 TestResult rows with
    /// distinct testName = 1 and distinct ofTest = 1: 42 real cases wearing
    /// one name, and a tally that read "ran 9417" for 8,710 distinct tests.
    ///
    /// The identity and the join key are different facts. Both survive.
    #[test]
    fn a_dynamic_case_keeps_its_own_name_and_still_joins_its_registered_row() {
        let f = "directing/products/cards/tests/cards-api-permutation.test.ts";
        let rows = vec![row(f)];
        let names: Vec<String> = vec!["runs without throwing".into()];
        let ents: Vec<String> = vec!["test-cards-api-permutation".into()];
        let cases = vec![
            CaseResult { file_path: f.into(), test_name: "cards CLI — permutation matrix (#2245) add P1 runs without throwing".into(), result: "pass".into() },
            CaseResult { file_path: f.into(), test_name: "cards CLI — permutation matrix (#2245) add P2 runs without throwing".into(), result: "pass".into() },
            CaseResult { file_path: f.into(), test_name: "cards CLI — permutation matrix (#2245) move WIP runs without throwing".into(), result: "pass".into() },
        ];
        let (joined, unjoined) = join_cases(&cases, &rows, &names, &ents);
        assert_eq!(unjoined, 0);
        assert_eq!(joined.len(), 3);
        let identities: std::collections::HashSet<&str> = joined.iter().map(|(c, _, _)| c.test_name.as_str()).collect();
        assert_eq!(identities.len(), 3, "three cases ran, three names survive: {:?}", identities);
        for (c, ent, reg) in &joined {
            assert_eq!(ent, "test-cards-api-permutation", "all three answer for the one registered row");
            assert_eq!(reg, "runs without throwing", "and report WHICH row they answer for");
            assert!(c.test_name.ends_with(reg.as_str()), "the join key is a suffix of the identity");
        }
    }

    /// NEGATIVE PROOF (#3734): an EXACT match must report the registered name
    /// too, not an empty string. Otherwise the census joins on "" for every
    /// ordinary case and reads all 8,749 registered tests as never-ran — the
    /// silent phantom census Kade flagged before this was written.
    #[test]
    fn negative_proof_an_exact_match_still_reports_its_registered_name() {
        let f = "platform/tests/a.bats";
        let rows = vec![row(f)];
        let names: Vec<String> = vec!["one".into()];
        let ents: Vec<String> = vec!["e1".into()];
        let cases = vec![CaseResult { file_path: f.into(), test_name: "one".into(), result: "pass".into() }];
        let (joined, _) = join_cases(&cases, &rows, &names, &ents);
        assert_eq!(joined[0].2, "one", "an exact match reports its own name as the join key, never empty");
        assert_eq!(joined[0].0.test_name, "one");
    }
...
mod integration_tier_4139 {
    ...
    fn negative_proof_backslash_case_joins_its_registered_row() {
        ...
        assert_eq!(joined[0].2, r"escapes newlines to literal \n");
        assert_eq!(joined[0].0.test_name, r"escSparql escapes newlines to literal \n");
    }
