...
//! owl-api HARD-REQUIRES a single `urn:chorus:ontology` graph for SHACL shapes
...
//! until owl-api can union shapes across domain graphs. Silas owns that
...
//! Silas confirmed the read path permits it (owl-api resolves shape scope
...
/// The single graph owl-api reads every shape and class definition from.
...
/// because owl-api reads schema from exactly one graph.
    ...
    if layer == Layer::Schema {
        return match declared {
            ...
            Some(d) => Err(Refusal::Placement(format!(
                "{}: schema (shapes, class defs, definesVocabulary) must live in {} — owl-api reads \
...
pub fn deploy_set_from_script() -> BTreeSet<String> {
    ...
    let path = format!("{}/platform/scripts/chorus-model-deploy.sh", root);
...
mod tests {
    ...
    // one-rule-for-both version would refuse ProductShape landing where owl-api
