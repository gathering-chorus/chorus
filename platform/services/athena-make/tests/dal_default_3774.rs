//! #3774 AC1 — athena-make's write delegation targets the LIVE DAL binary.
//!
//! #3718 retired `chorus-model` into a fail-loud stub (`athena-model` is the
//! live binary, same verbs). athena-make's dal_run default was never repointed, so
...
fn dal_default_is_the_live_binary_not_the_retired_stub() {
    ...
    assert_eq!(athena_make::dal_bin(), "/tmp/some-stub");
    std::env::remove_var("CHORUS_MODEL_BIN");
}
