...
use athena_make::coerce_effective;
...
// into ?v (athena-make's single-var ethos). value is LAST so an arbitrary config
// value may itself contain '|' — splitn(4) gives it the remainder.
use athena_make::parse_property_row;
...
use athena_make::{build_scope_node, decide_effective_value, ScopeKind};
...
use athena_make::{effective_fetch_query, NS};
...
use athena_make::effective_response;
