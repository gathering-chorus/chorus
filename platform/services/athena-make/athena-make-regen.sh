#!/usr/bin/env bash
# #3488 — regenerate a class's athena-make surfaces and LAND them at the
# model-declared repo location. The location is config-as-data: athena-make reads
...
#
# Usage: athena-make-regen.sh [Class]   (default: Domain)
# Env:   OWL_API_BIN (default: athena-make on PATH), CHORUS_FUSEKI, CHORUS_ROOT
set -euo pipefail

CLASS="${1:-Domain}"
BIN="${OWL_API_BIN:-athena-make}"
...
if [ -z "$DIR" ]; then
  echo "athena-make-regen: no repo target resolved for $CLASS" >&2
  exit 1
fi
DEST="$ROOT/$DIR"
mkdir -p "$DEST"
