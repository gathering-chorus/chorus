...
//! answers every SPARQL query owl-api issues with fixture rows, a stub DAL binary
...
use owl_api::{generate, serve, RouteTable};
...
fn stub_handle(stream: &mut TcpStream) {
    ...
    let req = owl_api::read_http_request(stream, 1 << 20);
...
fn world() -> &'static World {
    WORLD.get_or_init(|| {
        ...
        // ALL env, set once, before any owl-api logic runs (hermetic world).
        ...
        assert!(ready, "hermetic owl-api serve did not come up on :{}", port);
...
fn generate_verb_projects_the_generation_gap_seam() {
    let _ = world();
    let code = owl_api::generate_verb("athena-deploy").expect("verb projects");
    ...
    let e = owl_api::generate_verb("nope").unwrap_err();
...
fn product_index_and_domain_vocab_project_from_edges() {
    let _ = world();
    let idx = owl_api::generate_product_index("athena").unwrap();
    assert_eq!(
        idx,
        "{ \"product\": \"athena\", \"domains\": [{ \"name\": \"borg\", \"api\": \"/borg\" }, { \"name\": \"cards\", \"api\": \"/cards\" }] }"
    );
    let tables = owl_api::generate_domain_vocab("athena").unwrap();
    assert_eq!(tables.len(), 2, "athena definesVocabulary Domain+Product");
    let vocab = owl_api::all_vocab_classes().unwrap();
    assert_eq!(vocab, vec!["Domain".to_string(), "Product".to_string()]);
    let surfaces = owl_api::read_domain_surfaces().unwrap();
...
fn write_lifecycle_create_replace_edge_delete() {
    ...
    let big = format!("{{\"name\":\"n3\",\"comment\":\"{}\"}}", "x".repeat(owl_api::MAX_WRITE_BYTES));
...
fn effective_config_read_resolves_and_coerces() {
    let w = world();
    let (c, b) = owl_api::handle("/effective/pulse/alert.threshold", &w.domain);
    assert_eq!(c, 200, "{}", b);
    assert!(b.contains("\"value\":42"), "typed int, not a string: {}", b);
    assert!(b.contains("\"winningScope\""), "{}", b);
    // unset key → 404
    let (c, b) = owl_api::handle("/effective/pulse/nope.key", &w.domain);
    assert_eq!(c, 404);
    assert!(b.contains("no property sets key"), "{}", b);
    // hygiene guards
    let (c, _) = owl_api::handle("/effective/bad$node/k", &w.domain);
    assert_eq!(c, 400);
    let (c, b) = owl_api::handle("/effective/pulse/bad$key", &w.domain);
    assert_eq!(c, 400);
    assert!(b.contains("invalid key"), "{}", b);
    // handle_meta /health short-circuit
    let ((c, b), meta) = owl_api::handle_meta("/health", &w.domain, false);
    assert_eq!(c, 200);
    assert!(b.contains("\"ok\": true"));
    assert_eq!(meta.route, "health");
    // unknown route lists the generated routes
    let (c, b) = owl_api::handle("/nonsense", &w.domain);
    assert_eq!(c, 404);
...
fn emitters_project_the_same_model() {
    let w = world();
    let dash = owl_api::dashboards_json(&w.domain);
    assert!(dash.contains("\"uid\": \"owl-api-domain\""), "{}", dash);
    assert!(dash.contains("silent-broken-chain watch"), "{}", dash);
    let tm = owl_api::tests_manifest(&w.domain);
    ...
    let page_p = owl_api::page_html(&w.product);
    assert!(page_p.contains("entity-renderer.js"), "other classes use the generic renderer: {}", page_p);
    let dal = owl_api::dal_skeleton_ts(&w.domain, &["urn:test:instances".to_string()]);
