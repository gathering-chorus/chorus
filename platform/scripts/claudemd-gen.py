...
# #2731: role_filter is rejected for the canonical write paths (generate / pipeline).
# Per-role partial regen is the source of the staleness deadlock — when one role's
# CLAUDE.md is rewritten without the others, the other two carry stale fragment-hash
# stamps and their next SessionStart deadlocks. The canonical operation is "regen
# all three." For read-only modes (check / diff / dry-run / validate / compat /
# history) per-role narrowing is still useful and remains supported.
WRITE_MODES = {"generate", "pipeline"}
if role_filter:
    ...
    if mode in WRITE_MODES:
        print(
            f"ERROR: per-role generate is rejected (#2731). Run claudemd-gen with no "
            f"role argument to regenerate all roles atomically.",
            file=sys.stderr,
        )
        sys.exit(2)
...
def build_header_lines(version, now, protocol_version, core_hash, role_fragments_hash):
    # #3288: the protocol-core / role-fragments sha stamps are retired with the
    # runtime stamp-compare — nothing parses them anymore. The chorus-prompt
    # line stays as human-readable provenance (which Werk version this file
    # was generated against; the ledger is the authority).
    _ = (core_hash, role_fragments_hash)
    return [
        f"<!-- GENERATED by claudemd-gen.sh | v{version} | {now} | DO NOT EDIT DIRECTLY -->",
        f"<!-- chorus-prompt: {protocol_version} -->",
    ]

# --- #3288: version ledger — the ONE home of the Werk version ---
#
# designing/claudemd/version-ledger.json is committed and append-only. Each
# entry pins {version, constituent_hash, core_hash, bump, date, card}. The
# live version is a pure function of the committed constituent set: hash the
# constituents, look up the ledger head. PROTOCOL_VERSION is a rendered cache
# of the head (context_cache.rs and the prompt header read it), written ONLY
# in `bump` mode — a plain generate (including the SessionStart defensive
# regen) never writes version state, so a version change cannot sit floating
# in canonical (AC5).
#
# Bump classification (deterministic by constituent class):
#   protocol_core fragment changed        → MINOR
#   any other constituent changed         → PATCH
#   CLAUDEMD_MAJOR=1 (role-interop break) → MAJOR (explicit, never inferred)

LEDGER_PATH = os.path.join(claudemd_dir, "version-ledger.json")

def compute_constituent_hash():
    """One hash over everything that defines the protocol: every fragment
    file the manifest names (core + role sections + reference), plus the
    manifest's structural keys (variables/roles/reference/protocol_core —
    NOT _build/changelog, which are internal counters)."""
    files = set(manifest.get("protocol_core", []))
    for rc in manifest.get("roles", {}).values():
        files.update(rc.get("sections", []))
    if manifest.get("reference"):
        files.update(manifest["reference"].get("sections", []))
    h = hashlib.sha256()
    h.update(_hash_fragment_set(sorted(files)).encode("ascii"))
    structural = {k: manifest.get(k) for k in ("variables", "roles", "reference", "protocol_core")}
...
def ledger_head():
    entries = load_ledger().get("entries", [])
    return entries[-1] if entries else None

def parse_semver(v):
    m = re.match(r'^(\d+)\.(\d+)\.(\d+)$', v)
    if not m:
        raise ValueError(f"version '{v}' is not MAJOR.MINOR.PATCH semver")
    return tuple(int(x) for x in m.groups())

def bump_semver(v, position):
...
def classify_bump(head, live_core_hash):
    if os.environ.get("CLAUDEMD_MAJOR") == "1":
        return "major"
    if head.get("core_hash") != live_core_hash:
        return "minor"
    return "patch"
...
# Resolve version state once per run (before any role assembly).
protocol_core_hash = compute_protocol_core_hash()
constituent_hash = compute_constituent_hash()
_head = ledger_head()

if _head is None:
    # Pre-ledger state: seed from the PROTOCOL_VERSION literal. An X.Y
    # literal (legacy) becomes X.Y.0. `bump` records the first entry.
    _seed = read_protocol_version()
    if re.match(r'^\d+\.\d+$', _seed):
        _seed = f"{_seed}.0"
    protocol_version = _seed
    if mode == "bump":
        append_ledger({
            "version": protocol_version,
            "constituent_hash": constituent_hash,
            "core_hash": protocol_core_hash,
            "bump": "seed",
            "date": datetime.now().strftime("%Y-%m-%d"),
            "card": card_ref or "unattributed",
        })
        write_protocol_version(protocol_version)
        print(f"  Version ledger seeded at {protocol_version}", file=sys.stderr)
elif constituent_hash == _head["constituent_hash"]:
    protocol_version = _head["version"]
else:
    protocol_version = _head["version"]
    if mode == "bump":
        _position = classify_bump(_head, protocol_core_hash)
        protocol_version = bump_semver(_head["version"], _position)
        append_ledger({
            "version": protocol_version,
            "constituent_hash": constituent_hash,
            "core_hash": protocol_core_hash,
        ...
        write_protocol_version(protocol_version)
        print(f"  Version bump ({_position}): {_head['version']} → {protocol_version}", file=sys.stderr)
    elif mode not in ("dry-run", "diff", "check", "validate", "check-version"):
        print(f"  NOTE: constituents changed since v{_head['version']} — "
              f"run `claudemd-gen bump <card>` to record the new version.", file=sys.stderr)

if mode == "check-version":
    # CI coherence gate (#3288): committed PROTOCOL_VERSION and committed
    # fragments must both agree with the ledger head — coherence by
    # construction; the runtime stamp-compare layer is retired.
    failures = []
    if _head is None:
        failures.append("version-ledger.json missing or empty")
    else:
        try:
            parse_semver(_head["version"])
        except ValueError as e:
            failures.append(str(e))
        if constituent_hash != _head["constituent_hash"]:
            failures.append(
                f"constituent hash {constituent_hash[:12]} != ledger head {_head['constituent_hash'][:12]} "
                f"(fragments changed without `claudemd-gen bump`)")
        if read_protocol_version() != _head["version"]:
            failures.append(
                f"PROTOCOL_VERSION '{read_protocol_version()}' != ledger head '{_head['version']}'")
    if failures:
        for f_ in failures:
            print(f"FAIL: {f_}")
        sys.exit(1)
    print(f"version coherent: {_head['version']} ({constituent_hash[:12]})")
    sys.exit(0)

if mode == "bump":
    mode = "generate"  # bump = record the version, then generate as normal
