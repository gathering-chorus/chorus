...
if [ -n "$AUTHZ_HELD_FILE" ] && [ -n "$AUTHZ_REQUIRED_FILE" ]; then
  ...
  # #4194 — a held scope is a Permission ROW (acl:Authorization: agent +
  # accessTo + mode), not a hasScope literal on a principal. This check was the
  # last reader of hasScope: after #4183 landed (16:48) it read 0 held scopes
  # and called all 29 surfaces unreachable (chorus-health 17:40).
