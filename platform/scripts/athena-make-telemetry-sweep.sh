#!/bin/bash
# athena-make telemetry retention sweep (#3354) — prunes dated telemetry files
...
find "$CHORUS_HOME/ops/logs" -name "athena-make-*.jsonl" -mtime +"$RETENTION_DAYS" -print -delete 2>/dev/null | sed 's/^/pruned: /'
echo "athena-make telemetry sweep done (retention ${RETENTION_DAYS}d)"
