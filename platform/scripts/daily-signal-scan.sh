...
flow_health() {
  ...
  #
  # #4085 — the board is allowed to be unreachable; the brief is not allowed to
  # vanish because of it. With CHORUS_ROOT pointing at a werk, "$CARDS" list
  # fails (the werk has no built dist), and under `set -euo pipefail` that
  # assignment killed the whole scan. Jeff got no morning brief at all, and
  # daily-signal-scan.bats reported "produces output file" RED — which reads as
  # "the scan is broken" when the truth is "the board could not be asked".
  # Reproduced exactly: CHORUS_ROOT=<werk> ... --dry-run exits 1 here, and the
  # same command with CHORUS_ROOT unset exits 0.
  local board_output board_reachable=1
  board_output=$(bash "$CARDS" list 2>/dev/null) || board_reachable=0
  if [ -z "$board_output" ]; then board_reachable=0; fi
  if [ "$board_reachable" -eq 0 ]; then
    echo "- **Board:** UNREACHABLE — flow numbers below are unavailable, not zero."
    echo "  \`$CARDS list\` returned nothing. Every count in this section would be a"
    echo "  fabricated zero, so the section stops here."
    echo ""
    return 0
  fi
...
golfball_detection() {
  ...
  # #4085 — same rule as flow_health: an unreachable board ends this section
  # honestly instead of ending the whole brief.
  local board_output board_reachable=1
  board_output=$(bash "$CARDS" list 2>/dev/null) || board_reachable=0
  if [ -z "$board_output" ]; then board_reachable=0; fi
  if [ "$board_reachable" -eq 0 ]; then
    echo "  - Board UNREACHABLE — no golfball reading. Absence of findings here"
    echo "    means the board could not be asked, not that there are none."
    echo ""
    return 0
  fi
...
main() {
  ...
  # #4085 — say written only when something was written. The redirection above
  # can fail (unwritable directory, full disk) and this script still exited 0
  # and logged "Report written", so a morning with no brief at all looked
  # identical to a healthy one. Proven: --output /nonexistent-dir/x.md exited 0
  # before this check and exits 2 after it.
  if [ ! -s "$OUTPUT" ]; then
    echo "daily-signal-scan: report NOT written to $OUTPUT (missing or empty) — no brief today, and this run is not a success" >&2
    exit 2
  fi
