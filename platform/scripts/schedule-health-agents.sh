...
cat <<'EOF'
SCHEDULE bedroom-health "*/30 * * * *" "Run $CHORUS_ROOT/platform/scripts/health-check-bedroom.sh --card and report any failures. If all OK, stay silent."
SCHEDULE fuseki-baseline "3 9 * * *" "Run $CHORUS_ROOT/building/products/convergence/fuseki-baseline.sh and report timing. Alert if any query exceeds 10 seconds."
