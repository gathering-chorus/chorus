...
AUTH=(); [ -n "${FUSEKI_USER:-}" ] && AUTH=(-u "${FUSEKI_USER}:${FUSEKI_PASSWORD:-}")
...
case "$MODE" in
  --go)
    curl -sf ${AUTH[@]+"${AUTH[@]}"} --max-time 300 -X POST "$UPD" --data-urlencode \
      "update=PREFIX c: <$NS> INSERT { GRAPH <$DST> { ?s ?p ?o } } WHERE { GRAPH <$SRC> { ?s a c:$CLASS ; ?p ?o } }" >/dev/null
  ...
  --prune)
    ...
    curl -sf ${AUTH[@]+"${AUTH[@]}"} --max-time 300 -X POST "$UPD" --data-urlencode \
      "update=PREFIX c: <$NS> DELETE { GRAPH <$SRC> { ?s ?p ?o } } WHERE { GRAPH <$SRC> { ?s a c:$CLASS ; ?p ?o } }" >/dev/null
