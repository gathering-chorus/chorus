...
describe('runEventloopProbe (worker: emit every block, throttle nudges, measure from outside)', () => {
  it('emits on every over-threshold probe, throttles nudges to the window', async () => {
    const emits: number[] = [];
    const nudges: number[] = [];
    let clock = 0;
    ...
    expect(emits).toHaveLength(4); // every block emits a spine record (cheap, real data)
