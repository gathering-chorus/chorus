 ...
 * #3039 — listSessions/getSession/getSessionLog are now async (off the event
 * loop); call sites await them. isValidSessionId stays sync.
 */
...
describe('session-replay — listSessions', () => {
  ...
  test('empty sessions dir returns empty list', async () => {
    const { listSessions } = load();
    expect(await listSessions()).toEqual({ sessions: [] });
  });

  test('returns sessions sorted by lastActivity descending', async () => {
    ...
    const { listSessions } = load();
    const ids = (await listSessions()).sessions.map((s: any) => s.sessionId);
    expect(ids).toEqual(['ses_3_ccc', 'ses_2_bbb', 'ses_1_aaa']);
  });

  test('skips files older than 7 days (MAX_SESSION_AGE_MS)', async () => {
  ...
  test('ignores non-JSON files in the directory', async () => {
    ...
    const { listSessions } = load();
    expect((await listSessions()).sessions.map((s: any) => s.sessionId)).toEqual(['ses_1_x']);
  });

  test('tolerates malformed JSON files (skip)', async () => {
    ...
    const { listSessions } = load();
    expect((await listSessions()).sessions.map((s: any) => s.sessionId)).toEqual(['ses_good_y']);
  });

  test('session file without meta is silently skipped', async () => {
    fs.writeFileSync(path.join(TMP, 'ses_meta_x.json'), JSON.stringify({ events: [] }));
    const { listSessions } = load();
    expect((await listSessions()).sessions).toEqual([]);
  });

  test('returns empty when sessions dir does not exist', async () => {
...
describe('session-replay — getSession', () => {
  ...
  test('returns null for invalid id', async () => {
    const { getSession } = load();
    expect(await getSession('bad-id')).toBeNull();
  });

  test('returns null when file does not exist', async () => {
    const { getSession } = load();
    expect(await getSession('ses_404_nope')).toBeNull();
  });

  test('returns null when file is malformed JSON', async () => {
    fs.writeFileSync(path.join(TMP, 'ses_bad_y.json'), '{ partial');
    const { getSession } = load();
    expect(await getSession('ses_bad_y')).toBeNull();
  });
});
...
describe('session-replay — getSessionLog', () => {
  ...
  test('returns log content when file exists', async () => {
    ...
    expect(await getSessionLog('ses_1_a')).toBe('log line one\nlog line two');
  });

  test('returns null for invalid id', async () => {
    const { getSessionLog } = load();
    expect(await getSessionLog('nope')).toBeNull();
  });

  test('returns null when log file absent', async () => {
    const { getSessionLog } = load();
    expect(await getSessionLog('ses_404_abc')).toBeNull();
  });
});
