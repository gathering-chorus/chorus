...
export function resolvePulseSecret(): string | null {
  ...
  const p = secretPath();
  try {
    // eslint-disable-next-line security/detect-non-literal-fs-filename -- `p` is the fixed internal secret-file path (CHORUS_HOME / ops env override via secretPath()), never caller input — no path-traversal vector.
  ...
  try {
    // eslint-disable-next-line security/detect-non-literal-fs-filename -- internal secret-file path (secretPath()), not caller input.
