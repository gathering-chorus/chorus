...
describe('Origin label config', () => {
  test('LABELS.origin exists with reflective and reactive', () => {
    ...
    expect(LABELS.origin['reflective']).toBe(109);
    expect(LABELS.origin['reactive']).toBe(110);
  });
  ...
  test('origin label IDs are unique across all categories', () => {
    ...
    expect(counts.get(109)).toBe(1);
    expect(counts.get(110)).toBe(1);
  });
});
...
describe('Origin label in Vikunja', () => {
  ...
  test('origin:reflective label exists in Vikunja with ID 109', () => {
    const label = allLabels.find(l => l.id === 109);
    expect(label).toBeDefined();
    expect(label!.title).toBe('origin:reflective');
  });

  test('origin:reactive label exists in Vikunja with ID 110', () => {
    const label = allLabels.find(l => l.id === 110);
    expect(label).toBeDefined();
...
describe('SDK accepts origin as set key', () => {
  test('setCard does not throw on origin key', async () => {
    ...
    await expect(
      setCard(client, 2087, { origin: 'reflective' })
