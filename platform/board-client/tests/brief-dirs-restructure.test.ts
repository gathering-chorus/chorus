...
describe('#1802: Brief directories use canonical paths', () => {
  test('platform/roles/silas/briefs exists', () => {
    expect(fs.existsSync(path.join(PROJECT_ROOT, 'platform/roles/silas/briefs'))).toBe(true);
  });

  test('platform/roles/kade/briefs exists', () => {
    expect(fs.existsSync(path.join(PROJECT_ROOT, 'platform/roles/kade/briefs'))).toBe(true);
  });

  test('platform/roles/wren/briefs exists', () => {
    expect(fs.existsSync(path.join(PROJECT_ROOT, 'platform/roles/wren/briefs'))).toBe(true);
  });
