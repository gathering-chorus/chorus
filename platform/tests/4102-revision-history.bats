#!/usr/bin/env bats
# @test-type: integration — the live-stack cases are commented out while the
# file is held UNMEASURED, so the signals that would prove the declaration are
# temporarily absent; they come back with the body on the unhold.
#
# #4102 revision history — HELD by #4265, 2026-09-21 (Wren).
#
# Every case here calls GET /revisions. That route does not exist and never has:
# chorus:Version (chorus:Revision before #4211 renamed it) is DECLARED in the
# model but claimed by no domain, so athena-make serves no collection for it.
# /revisions and /versions both 404. Measured today, and measured before #4266
# moved the rows — the migration did not break this, it was never served.
#
# The rows are real: 16,993 of them in urn:chorus:domains:provenance. What is
# missing is the claim that mounts the surface, which is a model change in the
# provenance domain — Silas's file, his card, filed 2026-09-21.
#
# Held rather than deleted, because the feature is real and the cases are right.
# Held rather than skipped, because a skip that counts as a pass is the thing we
# are not allowed to write (#3734): this file reports UNMEASURED and exits 2, so
# a runner cannot read it as green.
#
# UNHOLD: when /versions serves, delete this banner, restore the body below, and
# point revisions_of at /versions.

echo "UNMEASURED: /revisions is unserved — chorus:Version is claimed by no domain." >&2
echo "  Not a red: the 16,993 rows exist; the collection does not. Blocked on the" >&2
echo "  provenance-domain card. This file scores as measured-nothing, never pass." >&2
exit 2

# ---------------------------------------------------------------------------
# HELD BODY — restored verbatim when the route serves.
# ---------------------------------------------------------------------------
# #!/usr/bin/env bats
