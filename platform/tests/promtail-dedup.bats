...
PROMTAIL_CONFIG="/Users/jeffbridwell/CascadeProjects/shared-observability/config/promtail/promtail-native.yaml"
LOKI="http://localhost:3102"

@test "Promtail config is valid" {
  /Users/jeffbridwell/bin/promtail -config.file="$PROMTAIL_CONFIG" -check-syntax 2>&1 | grep -q "Valid config"
}
...
@test "no recent daemon-logs entries for chorus-api.log (last 1 min)" {
  count=$(curl -s "${LOKI}/loki/api/v1/query" \
    --data-urlencode "query=count_over_time({job=\"daemon-logs\",filename=\"/Users/jeffbridwell/Library/Logs/Chorus/chorus-api.log\"}[1m])" \
