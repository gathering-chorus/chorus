...
OUT1=$("$BIN" add --kind role --name dal-e2e-sacrificial \
  --field label="DAL e2e sacrificial" --field comment="created+deleted by athena-model-e2e.sh (#3257)" 2>&1)
case "$OUT1" in
...
"$BIN" add --kind role --name dal-e2e-sacrificial \
  --field label="DAL e2e sacrificial" --field comment="created+deleted by athena-model-e2e.sh (#3257)" >/dev/null 2>&1
