...
setup() {
  ...
  row_code() { curl -s -o /dev/null -w '%{http_code}' "$URL/code/files/$NAME"; }
}

teardown() {
  [ -n "${NAME:-}" ] && curl -s -o /dev/null -X DELETE -H "Authorization: Bearer ${TOKEN:-}" "$URL/code/files/$NAME" || true
}
...
@test "a test file on disk becomes a row with kind=test, through the API, in the code domain graph" {
  ...
  [ "$status" -eq 0 ]
  [ "$(row_code)" = 200 ]
  run curl -s "$URL/code/files/$NAME"
...
@test "the walker writes through whichever door this server has — batch when #4158 is live, one POST per row before it" {
  ...
  if curl -s "$URL/code/files/openapi.json" | grep -q '/batch'; then
...
@test "NEGATIVE PROOF: a file the model has no kind or language for is skipped, never written as 'code'" {
  ...
  [ "$(curl -s -o /dev/null -w '%{http_code}' "$URL/code/files/$N")" != 200 ]
}
