...
@test "no script claims a LaunchAgent that does not exist" {
  # The reachability question this file failed to ask for months: a suite
  # asserting an absent plist is telling you the thing is unscheduled. Same
  # missing-denominator shape as the coverage floors — the system can enumerate
  # what it covers and cannot enumerate what it SHOULD.
  #
  # Running it the first time found THREE, not one. nightly-coverage.sh is retired
  # by this card. The other two are not mine and I have not verified them dead —
  # a script can legitimately reference a plist that simply is not installed on
  # this machine, and deleting someone else's script on that inference is exactly
  # the leap this card exists to stop.
  #
  # So they are ALLOWLISTED WITH A REASON rather than silently excluded. An
  # allowlist entry is visible debt; a deleted assertion is not. Raised to their
  # owners; each entry comes out when its owner rules retire-or-schedule.
  local ALLOW="com.chorus.bridge-subscriber-watchdog.plist com.chorus.werk-sweep.plist"
  ...
  while IFS= read -r f; do
    ...
    while IFS= read -r p; do
      ...
      [ -f "$HOME/Library/LaunchAgents/$p" ] || missing="$missing $p(→$(basename "$f"))"
    done < <(grep -o "com\.chorus\.[a-z-]*\.plist" "$f" | sort -u)
  done < <(grep -rl "com\.chorus\.[a-z-]*\.plist" "${CHORUS_ROOT}/platform/scripts" 2>/dev/null || true)
  [ -z "$missing" ] || { echo "scripts reference absent LaunchAgents:$missing"; false; }
}

@test "the reachability allowlist still fires on an unallowlisted orphan" {
  # Negative proof for the guard above (#3734 AC5): an allowlist that swallowed
  # everything would pass forever and prove nothing. Plant a script referencing a
  # plist that is neither installed nor allowlisted and assert the sweep sees it.
  ...
  while IFS= read -r f; do
    ...
    while IFS= read -r p; do
      [ -n "$p" ] || continue
      case " $ALLOW " in *" $p "*) continue ;; esac
      [ -f "$HOME/Library/LaunchAgents/$p" ] || missing="$missing $p"
    done < <(grep -o "com\.chorus\.[a-z-]*\.plist" "$f" | sort -u)
  done < <(grep -rl "com\.chorus\.[a-z-]*\.plist" "$probe" 2>/dev/null || true)
