...
@test "inside a werk with DEPLOY_TARGET=canonical said on purpose, the guard passes" {
  run env -u FUSEKI_GSP DEPLOY_TARGET=canonical CHORUS_ROOT="$WERK_DIR" bash -c "cd '$WERK_DIR' && '$SCRIPT'"
  [ "$status" -eq 0 ]
}
...
@test "inside a werk, a test naming its own throwaway ONTOLOGY_GRAPH passes (the #3601 discipline)" {
  run env -u FUSEKI_GSP -u DEPLOY_TARGET ONTOLOGY_GRAPH="urn:chorus:ontology-test-bats-x" CHORUS_ROOT="$WERK_DIR" bash -c "cd '$WERK_DIR' && '$SCRIPT'"
  [ "$status" -eq 0 ]
}

@test "negative proof: inside a werk, ONTOLOGY_GRAPH set to the LIVE graph is still REFUSED" {
  run env -u FUSEKI_GSP -u DEPLOY_TARGET ONTOLOGY_GRAPH="urn:chorus:ontology" CHORUS_ROOT="$WERK_DIR" bash -c "cd '$WERK_DIR' && '$SCRIPT'"
  [ "$status" -eq 78 ]
}
