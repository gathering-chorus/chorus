...
| Component | Function | Trigger | Key Files |
|-----------|----------|---------|-----------|
| `board-ts` | TypeScript Vikunja client — card CRUD, audit snapshots, staleness detection (Now >48h, Next >7d), WIP limiting | Manual, session-start.sh | `messages/board-client/src/cli.ts`, `messages/board-client/src/client.ts` |
