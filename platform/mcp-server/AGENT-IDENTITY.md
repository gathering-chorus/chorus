...
The ordinary stdio bridge verifies once at startup and again per tool invocation, rereading the token file each time. The operator-profile OpenCode bridge defers its first verification until the first tool call so OpenCode can initialize MCP before enrolling its native session; initialization and tool discovery perform no business operations. Expiration/rotation errors refuse operations; an already-running bridge never silently adopts a daemon credential. The file should be owned by the session operator and mode 0600. There is no automatic token renewal in this bridge; the identity/token owner must refresh the file.

For an OpenCode server created before session enrollment, the generated stdio configuration can supply `CHORUS_AGENT_BINDING_PROFILE` and `CHORUS_ROLE` instead of an initial session ID. The bridge POSTs `{profile,role}` to local `/v1/profile-binding`; the supervisor must return exactly one live session, never select newest or primary as a substitute. The first tool invocation pins its session ID, principal, and role, then verifies the CSS token against that session through the API. Calls made before enrollment fail closed without terminating discovery or adopting a fallback role. Each tool rechecks the binding and credential. A changed session/principal/role refuses with `profile-binding-changed-restart-bridge`; explicitly stop and restart the bridge for a replacement conversation. The OpenCode endpoint is restricted to one live session so server-scoped MCP config cannot ambiguously act for concurrent conversations.
