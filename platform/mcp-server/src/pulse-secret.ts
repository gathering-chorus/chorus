...
export function resolvePulseSecret(): string | null {
  ...
  try {
    // eslint-disable-next-line security/detect-non-literal-fs-filename -- secretPath() is a fixed internal path (CHORUS_HOME / ops env override), never caller input — no path-traversal vector.
