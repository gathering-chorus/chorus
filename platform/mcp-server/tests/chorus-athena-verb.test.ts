...
test('NEGATIVE PROOF — a red athena run is a thrown, named failure, never ok:true', async () => {
  await withServer(async (client) => {
    await assert.rejects(
      client.callTool({ name: 'chorus_athena', arguments: { role: 'wren', card_id: 4186, target: 'canonical', landed_commit: 'abc123' } }),
      (e: Error) => /chorus_athena-fail/.test(e.message) && /target=canonical/.test(e.message) && /stub says no/.test(e.message),
