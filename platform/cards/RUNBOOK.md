...
If ENOENT: check that `platform/cards/dist/` exists (`npm run build`).
If Vikunja unreachable: check `docker ps` for vikunja container, verify `.env` has the Vikunja access token.

## Build

```bash
cd platform/cards && npm install && npm run build
```

## Test

```bash
cd platform/cards && npx jest --no-coverage
```
...
Product-specific alerts in `platform/cards/alerts/`. Silas's observability platform consumes spine events for cross-product correlation.

## Logs

Runtime logs emit to `platform/cards/logs/`. Loki collects from this path.
