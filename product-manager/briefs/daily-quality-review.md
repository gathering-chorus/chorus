# Daily Quality Review — 2026-07-21
...
**RED** — BLOCKED: `ts-jest` preset not found (node_modules incomplete). Day 40.
- 0 tests run. No change from 2026-07-20.
- **Action:** `npm ci` needed to restore node_modules across all packages.

## Lint (`directing/clearing`)
**RED** — BLOCKED: `@eslint/js` not found in root node_modules. Day 42.
...
**RED** — 154 type errors. **No change from 2026-07-20** (was 154). Day 14 at this count.
...
**RED** — BLOCKED: `ts-jest` preset not found. Day 40.
...
## Failure Delta (vs 2026-07-20)
- **NO NEW REGRESSIONS:** Build type errors held at 154 (no change). Day 14 unchanged.
- **UNCHANGED:** All 4 suites blocked by `ts-jest preset not found` — now day 40.
- **UNCHANGED:** Lint blocked (`@eslint/js`) — now day 42.
- **Primary blocker remains:** `npm ci` at repo root unblocks all suites, lint, and build in one step. Now **42 days unresolved.**
