# Daily Quality Review — 2026-07-19
...
**RED** — BLOCKED: `ts-jest` preset not found (node_modules incomplete). Day 38.
- 0 tests run. No change from 2026-07-18.
- **Action:** `npm ci` needed to restore node_modules across all packages.

## Lint (`directing/clearing`)
**RED** — BLOCKED: `@eslint/js` not found in root node_modules. Day 40.
...
**RED** — 154 type errors. **No change from 2026-07-18** (was 154). Day 12 at this count.
...
**RED** — BLOCKED: `ts-jest` preset not found. Day 38.
...
## Failure Delta (vs 2026-07-18)
- **NO NEW REGRESSIONS:** Build type errors held at 154 (no change). Day 12 unchanged.
- **UNCHANGED:** All 4 suites blocked by `ts-jest preset not found` — now day 38.
- **UNCHANGED:** Lint blocked (`@eslint/js`) — now day 40.
- **Primary blocker remains:** `npm ci` at repo root unblocks all suites, lint, and build in one step. Now 40 days unresolved.
