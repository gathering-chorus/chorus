...
Last updated: 2026-09-03 12:20 Boston

## WIP
- **#4093 service page, engineering half** — werk kade-4093. Jeff 11:46 "our service designs seem rather untechnical, more like a pm view"; 11:49 "starts to look a bit like the domains tab for tests"; 11:56 "the domains ultimately will show true state". Built: facets.js = the ONE facet renderer (extracted from domain.html, both pages use it; a page defining its own FACETS table is the negative proof); service.html gains an Engineering half: Flows (chorus:diagram via the collection row, vendored mermaid, mirrors Wren's #4094 product render), Runs as (serviceinstances + scheduledjobs by runsService), and the hosting domain's facets composed via the read-only SPARQL door (Interfaces=API Contract, Data=Persistence, Depends on, Tests, Code, Logs, Alerts); the domain Tests facet is registry-first (GRAPH domains:tests, files once with layer, total = cases) with scanner fallback and proofs; nightly-suites + daily-review-summary map to service-tests; service-tests carries two diagrams (as-is, to-be) on the served row. Flows renders once #4094 (Wren) lands the shape property. Next: commit, pipeline, Chrome check of ?s=service-tests, Wren gate-product.
- **#4089 landed + accepted 12:03** (merge c57357fc5): tests service design as 35 Commitment rows in domains:services; Commitment claimed by services (Jeff); harvester replaces only its classes; seed --deploy per-home graphs; prod needed an athena-make restart to serve the new claim (landed ≠ running).
