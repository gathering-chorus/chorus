...
Last updated: 2026-09-02 09:20 Boston

## WIP
- **#4060 nightly readout** — werk kade-4060, branch kade/4060 at 870b5f61b, pipeline run 4060-kade-1788354684865-3217-91 to the demo. Morning: 4 plumbing commits (row-level pass/fail guard, stale envelope paths, werk store admin auth, api variant issuer+JWKS — Silas's #4058 blockers). Then the AC: `nightly-readout.ts` (one buildReadout: minutes, suites, reds+owners, newly-red/fixed/still-red/gone vs previous run), routes `/api/chorus/nightly/runs` + `/runs/:id[?format=text]`, `/nightly?run=<id>` with history list, `deliver_readout` nudges jeff from that route after the run (says "unavailable", no numbers, if the api is down), `--readout [id]` for roles. Tests: 19 jest + 6 bats, negative proofs on unknown id / api down / 404. Waiting on the demo, then Jeff's go.
