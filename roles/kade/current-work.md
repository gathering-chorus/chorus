...
Last updated: 2026-09-03 10:40 Boston

## WIP
- **#4089 tests service design in the graph** — werk kade-4089. Jeff 09:34 "move tests service design into services graph and revise deeply"; 10:07 "Commitment is 100% part of services"; Wren 10:10 (B): rows live in urn:chorus:domains:services, fix the harvester that wipes it. Done in werk: 35 Commitment rows for chorus:service-tests (tests-commitments-4089.ttl, revised against what runs: 15 closed w/ probes where one exists, 19 open (7 carded), 1 deferred); Service row service-tests; Commitment class+shape moved under services-domain definesVocabulary, instancesGraph urn:chorus:domains:services; athena-model `commitment` kind + seed --deploy lands each kind in its shape's home graph (one batch per home, manifest order); manifest carries card stubs + both commitment files; SECURITY_SET no longer loads 4064 rows; service-harvest-load.sh replaces only the classes it emits (one SPARQL UPDATE) with 4 proofs incl. the old wholesale form losing the row; commitment-check runs probes by kind; June + August HTML designs deleted, links → /athena/service.html?s=service-tests. Next: commit, pipeline, Wren reviews harvester diff, Silas gate-arch.
- Landed today: **#4063** (tests-domain never-ran 289→149 + api watermarks boot crash; merge 86d41a997, Jeff's go 08:03).
