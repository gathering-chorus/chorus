...
Last updated: 2026-09-03 07:40 Boston

## WIP
- **#4063 reopened (tests-domain red)** — werk kade-4063, commit ab30dad26, pipeline run 4063-kade-1788435666637-3217-162 to the demo. The 06:00 census: 289 registered tests never stored a result. Measured causes (full diff of registry vs 378k-row ledger): 113 shell scripts ran but store no row; 36 mcp-server node:test cases whose TAP titles escape `#` as `\#` so they never join; 22 cargo cases whose fn name lives in several hook modules (refused as ambiguous); 58 registered files no lane runs (27 js sexuality-player, 14 .feature, 13 playwright .cjs, 4 py); 28 ts cases inside platform/api suites that fail with "no such table: watermarks" (Silas's red); ~16 bats fixture names + a few template-literal names. Fixed in the werk: shell rows, TAP unescape, module-path cargo join, census lists all 289 with a by-kind line, readout labels the counted row a red not UNMEASURED. Expected after land: 289 → ~118, the rest named by kind. Wren's #4080 env-up (werk store has 0 Principal rows → seed refused) is next after this lands.
