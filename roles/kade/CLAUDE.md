...
### Services run as native LaunchAgents

The app, Fuseki, and CSS all run as native LaunchAgents. Docker is not used for any service (retired per #2020, 2026-04-14). Use `app-state.sh` and `launchctl kickstart`, not Docker.
...
Normal dev commands (npm, git, node) and `app-state.sh` for all lifecycle operations.
