# Brief: Move Silas to roles/silas — DEC-1816 namespace
...
Move `platform/roles/silas/` → `roles/silas/`. Delete the `platform/roles/architect` symlink. Update all references.
...
2. Move `platform/roles/silas/` → `roles/silas/`
3. Delete `platform/roles/architect` symlink (dangling alias — hides complexity, Jeff's words)
4. Grep for `platform/roles/silas` across the repo and update references
