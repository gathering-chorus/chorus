...
### G7: WIP Entry Blast Radius (ACTIVE — board-ts)
- **Mechanism**: `board-ts move <id> WIP` auto-generates blast radius via codebase graph query
- **Scope**: All roles, all cards entering WIP
- **Rules enforced**: 1 (DEC-072 — map what a card touches before it enters WIP)
- **Status**: Active since 2026-03-07 (#1098 shipped tool, #1086 wired enforcement)
- **Behavior**: Non-blocking enrichment. Queries `/api/codebase/topology` + `/api/codebase/node/*`, classifies files into 6 dimensions (UI/UX, API, OWL/RDF, Page pattern, Tests, Harvests), posts structured comment on card, emits `card.blast_radius.generated` spine event. On failure, emits `card.blast_radius.failed` spine event (visibility, not block).
- **Audit**: Spine events in Chorus index. Card comments persist on board. Failure events visible in Loki.
