...
1. **Zero-dep blueprint** (werk-pull #3045 `lib.rs`): thin main (exit 0 print / exit 1 eprintln); pure unit-tested helpers; typed `run()`/`run_in()` subprocess wrappers; `FlockGuard` + `lock()`; a testable core `verb(card, role, home, werk_base, …)` with ALL inputs explicit; `run_verb()` parses args/env; `R<T> = Result<T, String>`; all-or-nothing `rollback()` under the lock; best-effort `jsonl()` witness that never affects the op. **ZERO chorus-code deps** — std + libc(flock) + CLI subprocesses (git/gh/cargo/codesign/launchctl/build-signed.sh/chorus-bin-install/npm/sh/shasum/cp/curl) only; no chorus-log, no chorus-sdk. (werk-pull's `Cargo.toml [dependencies]` is empty — the defining axis, CI-checkable.) **Orchestrating verbs (demo) invoke the sibling verb binaries (werk-build / werk-deploy / werk-verify) as subprocesses** — still zero-dep (a subprocess, not a code dependency) — so an orchestrating verb's allowed-subprocess set is the union of git/gh/cards plus those binaries. A verb's subprocess list must cover what it actually shells out to.

   **#3092 amendment (2026-05-26) — multi-language coverage:** werk-build and werk-deploy ship as single verbs that walk the branch diff and dispatch each buildable/deployable thing to its own per-language path. werk-build's `discover_build_units` yields a `BuildUnit::RustCrate(...)` for changes under `platform/services/<crate>/` and a `BuildUnit::TsService(...)` for changes under `platform/api/`; each unit is built with its own command (`cargo`+`codesign` via `build-signed.sh` for Rust, `npm run build` for TS). werk-deploy's `target_class(name)` resolves each name to a `RustService` (kickstart + cdhash verify), `TsService` (dist install + LaunchAgent kickstart + health smoke + dist-sha verify), or `CliVerb` (install + installed-cdhash verify, no kickstart). Adding a new language or service type = one new detection rule, not a verb rewrite — the work IS the data (Jeff's framing). Subprocess set widened by `npm` (TS build), `cp -R` (dist install), `sh` (find/shasum pipeline for dist hash), `curl` (health smoke).
...
5. **gh-wiring (#3056 AC#4):** per-verb commit-status `chorus/<verb>/<card>` carrying the verb's relevant **stable identity hash** (cdhash for Rust binaries, sha256-of-dist for TS services, sha for commit); **carry prior `chorus/*/<card>` statuses forward** onto the new SHA; one shared trace across gh + jsonl. **commit-status, NOT gh DEPLOYMENTS** — we have one environment and it is production, so DEPLOYMENTS' multi-env modeling is wasted and commit-status keeps uniformity + the carry-forward mechanic.

   **#3092 amendment (2026-05-26) — identity-hash generalization:** earlier revisions said "cdhash" because the only buildable was Rust binaries. The contract is actually a **stable identity hash**: same source → same hash, available pre-install so deploy can refuse a stale build, and re-checkable post-install for running==built verification. Two concrete forms today: **cdhash** for Rust binaries (codesign-derived, TCC-stable) and **sha256(dist tree)** for TS services (proven deterministic — three back-to-back tsc builds yield byte-identical dist sha). The build/deploy contract reads identical for both; only the form differs. Future languages add a third form by extending the rule, not the contract.
