...
### 5. Per-agent credentials, model-resolved registry
The four agent WebIDs already exist as pod profiles (`/pods/jeff/_agents/{wren,silas,kade,bridge}/profile/card.ttl#me`, ADR-042). Each agent gets its **own** CSS client credential, **not settable from that agent's own shell env** (minting requires the credential, which the agent's environment does not carry as a forgeable string). The static phase-1 WebID registry becomes model-resolved (#3573 phase-2): adding or revoking an agent is a model edit. Revocation is real — revoke a credential, its writes refuse within one token TTL.
