...
- **clearing** (parent product, Wren): domains `cards`, `priorities`, `messages`, `heralds`, `senses`.
  - `priorities` folds into `cards` as the board's upper register (verbs persist as skills); not yet a graph domain — Wren mints it via DAL on her #3351 leg.
  - `heralds` = delivery **contracts + skills** over the ONE delivery engine; it is **never a second delivery path** (Wren's pin). The engine lives in pulse/spine; heralds names the contract.
