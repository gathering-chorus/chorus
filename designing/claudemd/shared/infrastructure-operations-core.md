...
Never kill PIDs manually. Views/CSS are served directly from disk (no deploy). TypeScript changes need a `deploy` verb on the appropriate script. Logs via Loki (`localhost:3102`), not by tailing service files.
