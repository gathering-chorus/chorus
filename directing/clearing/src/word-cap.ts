...
export function countWords(text: string): number {
  // #3851 — the chorus prompt header is METADATA and never counts.
  //
  // Jeff, 2026-08-13: "dont count any metadata. its just whatever words you
  // need to make that are specific to me." The header (--- Role | time | card |
  // Werk vN ---) is required on every response and is not addressed to him, so
  // charging ~12 words of it against a 100-word budget taxes the message for
  // obeying a different rule.
  //
  // LEADING only. A dashed line in the middle of prose is content — stripping
  // every --- ... --- line would let anyone hide a paragraph inside dashes.
  const withoutHeader = text.replace(/^\s*---[^\n]*---\s*(\n|$)/, ' ');
  const withoutFences = withoutHeader.replace(/```[\s\S]*?```/g, ' ')
