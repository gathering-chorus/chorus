...
### Shared observation mode (the primary pattern)

Tail is most powerful as a **shared observation window** between Jeff and the current role. The pattern:

1. Jeff says `/chorus tail kade` (or another role)
2. You show what that role is doing right now
3. **Jeff and you discuss what you see** — assess their work, spot issues, make decisions
4. You can **act on what you observe** — restart a service, wake a volume, send a brief — without interrupting the other role's session

This is **observe → assess → decide → act** in real-time. Example: Jeff and Silas watched Kade debug a video-server hang, Silas assessed Kade's diagnosis ("yes, both infra and code issues"), Jeff decided to wake the volumes, Silas acted — all while Kade kept working uninterrupted.

**Key behaviors in this mode:**
- **Do analyze and assess** what you see — Jeff is asking for your perspective, not a raw feed
- **Offer to act** when you see something in your domain (infra issue, architectural concern, brief needed)
- **Be direct** about agreement or disagreement with the other role's approach
- **Stay in your lane** — observe and advise, don't reach into the other role's session
...
5. **To stop**: use TaskStop on the background task ID. Print a brief summary: how long the tail ran and how many batches surfaced.
