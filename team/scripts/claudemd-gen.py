...
def validate_manifest(manifest, claudemd_dir, verbose=False):
    ...
    for role_name, role_config in roles.items():
        ...
        if "output" in role_config:
            messages_dir = os.path.dirname(claudemd_dir)
...
def resolve_output_path(output_rel):
    """Resolve output path relative to claudemd_dir's parent (messages/)"""
    messages_dir = os.path.dirname(claudemd_dir)
    return os.path.normpath(os.path.join(messages_dir, output_rel))
...
if mode == "generate":
    ...
    if version != old_version:
        print(f"  Auto-bump: v{old_version} → v{version}", file=sys.stderr)
    header_comment = f"<!-- GENERATED by claudemd-gen.sh | v{version} | {now} | DO NOT EDIT DIRECTLY -->"
