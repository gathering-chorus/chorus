...
### 2. Write demo brief to Wren

**Generate the demo brief now**, before the gate chain fires. This is the provenance record — it proves the builder completed AC before requesting gates.

```bash
BRIEF_PATH="/Users/jeffbridwell/CascadeProjects/chorus/roles/wren/briefs/$(date +%Y-%m-%d)-demo-${CARD_ID}.md"

cat > "$BRIEF_PATH" << EOF
# Demo ready: #${CARD_ID} — <title from card>

**Builder:** <owner from card>
...
Auto-generated by /demo pre-flight (#2068).
Ready for gate chain.
EOF
```

**Path must be `roles/wren/briefs/`** — use the role directory name, never the role description (`product-manager`).

**Gate logic:**
- If brief write fails → **STOP.** `Failed to write demo brief to ${BRIEF_PATH}.`
- If brief written → print: `Demo brief written: ${BRIEF_PATH}`
...
1. **Run `/gate-product <card-id>`** — AC described, experience section, domain registered, spine contract. Wren runs this. On pass, nudges Kade for gate-code. **Note:** The demo brief already exists from Step 1.5 (AC Pre-flight), so the "demo evidence" check should pass on first run.
...
The demo brief was already created in **Step 1.5 (AC Pre-flight)** with AC status. Now update it with smoke check results and stakes summary from Steps 3-4.

```bash
BRIEF_PATH="/Users/jeffbridwell/CascadeProjects/chorus/roles/wren/briefs/$(date +%Y-%m-%d)-demo-${CARD_ID}.md"

# Append smoke check + stakes to the brief created in Step 1.5
cat >> "$BRIEF_PATH" << EOF

## Smoke Check
**Result:** PASS (<N> pass, 0 fail)

## What to show Jeff
<1-2 lines from your Step 4 stakes summary>

## Gates
product PASS | code PASS | quality PASS | arch PASS | ops PASS
...
4. Demo brief exists in `roles/wren/briefs/` (created in Step 1.5, updated here)
